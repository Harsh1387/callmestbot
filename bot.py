...
async def ask_groq(channel_id: int, username: str, prompt: str) -> str:
    """Send a message to Groq and return the reply, keeping conversation history."""
    if not GROQ_KEY:
        return "❌ Groq API key not set. Add `GROQ_KEY` in Railway variables."

    # Build history for this channel (max last 10 messages to save tokens)
...
async def on_message(message):
    ...
    # AI channel mode — every message gets a Groq reply
...
async def ai_cmd(ctx, *, prompt: str):
    """Ask the AI a question anywhere."""
...
async def ai_reset(ctx):
    """Clear the AI conversation memory for this channel."""
...
async def ai_set(interaction: discord.Interaction):
    if interaction.channel_id not in DATA["ai_channels"]:
        ...
        save_data(DATA)
    await interaction.response.send_message("✅ AI mode enabled! Every message here will get an AI reply.")
